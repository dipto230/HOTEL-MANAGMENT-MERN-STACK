...
const AddRoom = () => {
  ...
  return (
    <form>
      ...
      <p className='text-gray-800 mt-20'>Images</p>
      <div className='grid grid-cols-2 sm:flex gap-5 my-2 flex-wrap'>
      ...
      <div className='w-full flex max-sm:flex-col sm:gap-4 mt-4'>
        <div className='flex-1 max-w-48'>
          ...
          <select value={inputs.roomType} onChange={e=>setInputs({...inputs, roomType:e.target.value})}
            className='border opacity-70 border-gray-300 mt-5 rounded p-2 w-full'>
        ...
        <div>
          ...
          <input type='number' placeholder='0' className='border border-gray-300 mt-5 rounded p-2 w-24' value={inputs.pricePerNight}
            onChange={e=>setInputs({...inputs, pricePerNight: e.target.value})}
          />
        </div>

      </div>
      <p className='text-gray-800 mt-5'>Amenities</p>
      ...
      <button className='bg-blue-800 text-white px-8 py-2 rounded mt-10 cursor-pointer'>
        Add Room
      </button>
     </form>
  )
